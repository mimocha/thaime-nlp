...
# Filter summary (order matters — cheapest checks first):
# 1. Length bounds: reject empty, single-char, and >30-char tokens
# 2. Thai-script-only: reject tokens containing digits, Latin, punctuation
# 3. Maiyamok-only: reject ๆๆๆ repetition artifacts
# 4. Leading maiyamok: reject ๆคน-style tokenizer fragments
# 5. Repeated chars (4+): reject spam like ดดดด, กกกก
# 6. No-consonant tokens: reject pure vowel/mark sequences like าาา, ะะ
# 7. Single repeating char: reject ดดด, กกก (legitimates like กก via overrides)
