...
### Layer 3: LLM semantic filter (`pipelines/trie/llm_filter.py`)

An offline tool (not part of the automated pipeline) that uses AWS Bedrock to semantically evaluate whether tokens are real Thai words. Produces candidate exclusions that the maintainer reviews and merges into the exclusion list.
